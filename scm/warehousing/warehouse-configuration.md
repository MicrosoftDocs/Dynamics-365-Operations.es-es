...
Las entidades que crea para definir el diseño del almacén se usan en las consultas que configura en las plantillas de trabajo para manejar los pedidos de trabajo en el almacén. Por lo tanto, al definir las zonas, los tipos de ubicación, etc., piense en cómo se usan diferentes áreas del almacén para distintos procesos. Además, considere factores como las características físicas de un área concreta. Por ejemplo, puede haber las áreas en las que puede usar si un tipo determinado de elevador de carga. O bien, si la empresa tiene la producción y producto terminado dentro de la misma facilidad, puede que desee crear un único almacén en Dynamics 365 para las operaciones pero por otra parte para separar las dos operaciones creando dos grupos de la zona. Dé a sus entidades los nombres de, de modo que sea fácil identificarlos al configurar usa en consultas de la plantilla.
...
La garantía de ayuda de los límites de la media de la ubicación que el trabajo no se ha creado para solicitar ese inventario colocado en una ubicación que no tenga la capacidad física de llevar el inventario. Por ejemplo, si algunas ubicaciones dentro de un almacén sólo pueden retener un pallet por ubicación, los límites de la media de la ubicación se pueden habilitar. ** Cantidad ** el valor se puede liquidar ** ** 1, unidad y ** ** el valor se puede liquidar ** PL ** dentro de una agrupación específica del perfil de ubicación. 
...
Para crear rápidamente las ubicaciones dentro de un almacén, puede usar ** ubicación configurar ** el asistente. Como parte de este proceso, puede mantener con facilidad el formato de los nombres de ubicaciones.
