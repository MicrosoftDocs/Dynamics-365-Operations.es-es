...
Considere la modificación después de la situación de ejemplo: Un equipo que una empresa está fabricando contiene un componente que caducarán del catálogo del proveedor en dos meses. A partir de la fecha de caducidad en adelante, el proveedor proporcionará un nuevo componente que se pueda sustituirá para el antiguo componente. Las fechas de inicio y fin de validez se pueden configurar en las líneas de la lista de materiales. Por ejemplo, puede configurar que el antiguo componente deje de existir especificando la fecha de vencimiento en el campo **Fecha final**. A continuación, en la lista de materiales puede configurar el nuevo componente de sustitución de forma que sea válido a partir del día después de que venza el componente antiguo. Para ello, especifique la fecha de inicio en el campo **fecha inicial**.
