...
Una previsión de la demanda se calcula para un artículo y sus dimensiones únicamente si el artículo es parte de una clave de asignación de artículos. Se aplica esta regla para agrupar un gran número de artículos, para poder crear previsiones de demanda con más rapidez. Se omite el porcentaje de la clave de asignación de artículos cuando se generan las previsiones de la demanda. Las previsiones se crean únicamente en base a los datos históricos. 
...
Para agregar una referencia de almacén (SKU) a una clave de asignación de artículos, vaya ** planificación maestra ** &gt; ** configuración ** &gt; ** previsión de demanda ** &gt; ** las claves de asignación de artículos **. Use la página **Asignar artículos** para asignar un artículo a una clave de asignación.

## <a name="intercompany-planning-groups"></a>Grupos de planificación de empresas vinculadas
La previsión de demanda genera previsiones entre empresas. En Microsoft Dynamics 365 para las operaciones, agrupan a empresas que se planifican conjuntamente en un grupo de planificación empresas vinculadas. Para especificar, por la empresa, qué claves de asignación de artículos se deben considerar para la previsión de demanda, asociar una clave de asignación de artículos al miembro empresas vinculadas del grupo de planificación por ** planificación maestra ** ** &gt; configuración que iba ** &gt; ** los grupos de planificación ** empresas vinculadas. 

De forma predeterminada, si no se ha asignado ninguna clave de asignación de artículos a los miembros empresas vinculadas del grupo de planificación, una previsión de la demanda se calcula para todos los artículos asignados a todas las claves de asignación de artículos de todas las Dynamics 365 para las empresas de operaciones. Las opciones de filtrado adicionales para las empresas y las claves de asignación de artículos están disponibles en la página **Generar la previsión estadística de línea base**. 
...
Para configurar parámetros de la previsión de demanda, vaya ** planificación maestra ** &gt; ** configuración ** &gt; ** los parámetros de la previsión de demanda **. Dado que la previsión de demanda se realiza entre empresas, la configuración es global. Es decir, la configuración se aplica a todas las empresas. 
...
Para generar la previsión, Dynamics 365 para las operaciones usa un servicio web de aprendizaje de equipo. Para conectar el servicio, deberá proporcionar Dynamics 365 para las operaciones la siguiente información si el iniciar sesión en Microsoft Azure Machine Learning Studio:
...
**Nota:** solo se requiere el nombre de la cuenta y la clave de almacenamiento de Azure si usa una cuenta personalizada de almacenamiento. Si implementa la versión local, deberá disponer de una cuenta personalizada de almacenamiento en azul, de modo que el servicio de aprendizaje de equipo pueda acceder a los datos históricos. 

Para crear predicciones de demanda, puede implementar su propio servicio mediante revisión de aprendizaje de equipo o Dynamics 365 para los unidad de previsión de demanda de las operaciones. Las instrucciones para implementar Dynamics 365 para la previsión de demanda de las operaciones experimentan como un servicio web es Dynamics 365 disponible para las operaciones. En la página **Parámetros de previsión de demanda**, haga clic en la pestaña **Aprendizaje automático de Azure**.

## <a name="settings-for-the-dynamics-365-for-operations-demand-forecasting-machine-learning-service"></a>Valores para Dynamics 365 para el servicio de aprendizaje de equipo de previsión de demanda de operaciones
Para ver los parámetros que se pueden configurar para Dynamics 365 para el servicio de previsión de demanda de operaciones, vaya ** planificación maestra ** &gt; ** configuración ** &gt; ** previsión de demanda ** &gt; ** los parámetros de algoritmo de previsión **. ** Parámetros de algoritmo de previsión ** a las pagine muestra los valores predeterminados para los parámetros. Puede anular los parámetros en ** los parámetros de la previsión de demanda ** la página. Use la pestaña **General** para sobrescribir los parámetros globalmente, o use la pestaña **Claves de asignación de artículos** para sobrescribir los parámetros por clave de asignación de artículo. Los parámetros que se sobrescriben para una clave de asignación de artículos afectan solo a la previsión de los artículos que están asociados con esa clave de asignación de artículos.
