...
Las transacciones de inventario se pueden actualizar físicamente y financieramente en Microsoft Dynamics 365 para las operaciones. Algunos tipos de transacciones financieras y físicas aumentan las cantidades de inventario, mientras que otros reducen la cantidad.
...
Las transacciones que aumentan la cantidad se registran en el precio de coste promedio móvil. La Dynamics 365 para las operaciones calcula un precio de coste promedio móvil basado en el coste de cada una de estas transacciones para cada dimensión de inventario que se efectúa un seguimiento financiero. Para obtener información acerca del precio de coste promedio móvil, consulte [Precio de coste promedio móvil](running-average-cost-price.md).

## <a name="transactions-that-decrease-quantity"></a>Transacciones que reducen la cantidad
La Dynamics 365 para las operaciones usa el precio de coste promedio móvil cuando se registra una transacción que reduce la cantidad, independientemente del modelo de inventario que está asociado a ese inventario. La transacción que reduce la cantidad no debe haberse marcado para otra transacción antes de registrarse. Si el inventario físico disponible sea negativo, Dynamics 365 para las operaciones usa el coste de inventario definido para el artículo en artículo ** ** la página. **Nota**: si la funcionalidad multisitio está activada, este coste será en su lugar el coste de inventario definido para un sitio en la página **Configuración predeterminada de pedido**.
