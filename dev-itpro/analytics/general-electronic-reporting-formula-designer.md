...
| DATETODATETIME (fecha)                      | Convertir el valor de la fecha especificada en un valor de fecha y hora.                                                                                                                                                                                                                                                                                                            | **DATETODATETIME (CompInfo. 'getCurrentDate()')** devuelve la fecha de la sesión actual de Dynamics 365 for Operations, 12/24/2015, como **12/24/2015 12:00:00 AM**. En este ejemplo, **CompInfo** es un origen de datos de ER del tipo **Dynamics 365 for Operations/Tabla** que hace referencia a la tabla CompanyInfo. |
...
| Función                                                                                | Descripción                                                                                                                                                                                                                                                                     | Ejemplo                                                                                                                                                                                                                                                      |
|-----------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| CASE (expresión, opción 1, resultado 1 \[, opción 2, resultado 2\] ... \[, resultado predeterminado\]) | Evaluar el valor de la expresión especificado con las opciones alternativas especificadas. Devolver el resultado de la opción que es igual al valor de la expresión. En caso contrario, devolver el resultado predeterminado introducido opcionalmente (el último parámetro al que no le antecede una opción). | **CASE( DATETIMEFORMAT( AHORA(), "MM"), "10", "INVIERNO", "11", "INVIERNO", "12", "INVIERNO", "")** devuelve la cadena **"INVIERNO"** cuando la fecha actual de la sesión de Dynamics 365 for Operations se encuentra entre octubre y diciembre. En caso contrario, devuelve una cadena en blanco. |
...
<td>Si el servidor local de la instancia de Dynamics 365 for Operations se define como <strong>EN-US</strong>, <strong>TEXT (NOW ())</strong> devuelve la fecha de la sesión actual de Dynamics 365 for Operations, 12/17/2015, como la cadena de texto <strong>&quot;12/17/2015 07:59: 23 AM&quot;</strong>. <strong>TEXT (1/3)</strong> devuelve <strong>&quot;0,33&quot;</strong>.</td>
</tr>
<tr class="even">
...
Esta es la fórmula que se puede diseñar: FORMAT (CONCATENATE (@&quot;SYS70894&quot;, &quot;. &quot;, @&quot;SYS18389&quot;), model.Customer.Name, DATETIMEFORMAT (model.ProcessingDate, &quot;d&quot;)) Si se procesa un informe para el <strong>cliente de Litware Retail</strong> el 17 de diciembre de 2015, en la referencia cultural <strong>EN-US</strong> y el idioma <strong>EN-US</strong>, esta fórmula devuelve el siguiente texto, que se puede presentar como mensaje de excepción para el usuario final: &quot;Nothing to print. Customer Litware Retail is stopped for 12/17/2015.&quot; Si el mismo informe se procesa para el<strong> cliente de Litware Retail</strong> el 17 de diciembre de 2015, en la referencia cultural <strong>DE</strong> y el idioma <strong>DE</strong>, esta fórmula devuelve el siguiente texto, que usa un formato de fecha diferente: &quot;Nichts zu drucken. Debitor 'Litware Retail' wird für 17.12.2015 gesperrt.&quot; <strong>Nota: </strong>La sintaxis siguiente se aplica en las fórmulas de ER para las etiquetas:
<ul>
<li><strong>Para las etiquetas de los recursos de Dynamics 365 for Operations:</strong> <strong>@&quot;X&quot;</strong>, donde la X es el identificador de la etiqueta en el Árbol de objetos de aplicación (AOT).</li>
<li><strong>Para las etiquetas que se encuentran en las configuraciones de ER:</strong> <strong>@&quot;ER_LABEL:X&quot;</strong>, donde X es el id. de etiqueta de la configuración de ER.</li>
</ul></td>
</tr>
<tr class="odd">
...
SUMIFS (cadena clave para sumar, cadena de criterio range1, cadena de criterio value1 \[ cadena de criterio range2, cadena de criterio value2, …\])
...
COUNTIFS (cadena de criterio range1, cadena de criterio value1 \[, cadena de criterio range2, cadena de criterio value2, …\])
...
COLLECTEDLIST (cadena de criterio range1, cadena de criterio value1 \[, cadena de criterio range2, cadena de criterio value2, …\])
...
| ROUNDAMOUNT (número, decimales, regla de redondeo)                                       | Redondee el importe especificado de acuerdo con la regla de redondeo especificada y el número especificado de decimales. **Nota:** La regla de redondeo se debe especificar como un valor de la enumeración **RoundOffType** de Dynamics 365 for Operations.                          | Si el parámetro **model.RoundOff** se establece en ***Hacia abajo****, **ROUNDAMOUNT (1000.787, 2, model.RoundOff)** devuelve el valor **1000,78**. Si el parámetro **model.RoundOff** se establece en **Normal** o **Redondeo por arriba**, **ROUNDAMOUNT (1000.787, 2, model.RoundOff)** devuelve el valor **1000,79**. |
...
| FA\_BALANCE (código de activo fijo, código del modelo de valor, año de notificación, fecha de notificación) | Devuelve el contenedor de datos preparado de los saldos de activos fijos. El año de notificación debe especificarse como valor de la enumeración de Dynamics 365 for Operations **AssetYear**.                                                                                           | FA\_SUM ("COMP-000001", “Actual”, AxEnumAssetYear.ThisYear, SESSIONTODAY ()) devuelve el contenedor preparado de los datos de los saldos del activo fijo “COMP-000001” con el modelo de valor “Actual” en la fecha de la sesión actual de 365 for Operations.                                                                |
