...
-   El área de los encabezados de la definición de la columna contiene el texto y el formato de encabezado que aparece en el informe. Una cabecera puede aplicar a una columna de datos, puede abarcar varias columnas, o puede aplicar a las columnas de una base condicional. La definición de columna puede incluir tantas filas de cabecera de columna como sea necesario. **Nota:** Los encabezados de columna se aplican a cada columna de datos del informe. Los encabezados de informes se aplican a todo el informe. Defina los encabezados de informes en la pestaña **Encabezados y pies de página** de la definición del informe.
...
|-------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| SU                      | Suprima el subrayado para una columna donde o un comando de subrayado (**---**) o se especifica un comando de subrayado doble (**===**) en la definición de filas. Por ejemplo, puede que no desee subrayar los importes que son producidos por un cálculo porcentual.                                                                        |
...
El diseñador de informes puede generar automáticamente encabezados de columna, en función de códigos de autotexto. Los códigos de Autotexto son variables que se actualizan cada vez que un informe se genera. Cualquier encabezado de columna puede incluir estos códigos para especificar la información de informe que puede variar, como fechas o números de período. Por lo tanto, puede usar una definición de columna para las definiciones de varios informes, períodos de tiempo y organigramas. Dado que los códigos de autotexto confían en la información del calendario de las filas de detalle de la definición de la columna, se admiten solo para las columnas **CALC****FD** y **WKS**. La forma en que un código de autotexto aparece en la celda de encabezado de columna afecta a cómo dicha información aparece en el informe. En el cuadro de diálogo **Encabezado de columna**, los códigos de autotexto aparecen en mayúscula y minúscula. Por lo tanto, el texto aparece en mayúsculas y minúsculas en el informe. Por ejemplo, en un año natural estándar, **@CalMonthLong** mes de las resoluciones ** 7 ** a ** ** julio. Si el nombre del mes (es mayúsculo por ejemplo JULIO ** **), escriba el código del autotexto en caracteres en mayúsculas en ** texto del encabezado de columna ** el campo. Por ejemplo, escriba ** **@CALMONTHLONG. Puede mezclar códigos y el texto. Por ejemplo, puede especificar el texto de encabezado siguiente: ** Período @StartDate @FiscalPeriod-@FiscalYear @StartDate @EndDate**. El encabezado del informe que se genera se asemeja al texto siguiente: **Período 1-02 desde 01/01/02 a 01/31/02**. ** Nota: ** El formato de algo de texto, como la fecha larga, depende de sus configuraciones regionales en Dynamics 365 para el servidor de las operaciones. Para cambiar estos ajustes, haga clic en el botón **Iniciar**, haga clic en **Panel de control**y luego en **Región y idioma**. En la tabla siguiente se enumeran las opciones disponibles de autotexto para encabezados de columna.

| Opción y código de Autotexto                | Descripción                                                                                                                                                                                                                                                                                      |
|-----------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Nombre de mes (@CalMonthLong)              | Imprimir el nombre del mes actual en el encabezado de columna. Si decide redondear los importes del informe en millares, millones o billones, o si establece la anchura de columna en el informe a menos de nueve caracteres, el nombre del mes se abrevia a los primeros tres caracteres. |
| Nombre abreviado de los meses (@CalMonthShort) | Imprimir el nombre abreviado del mes para el período fiscal seleccionado.                                                                                                                                                                                                                          |
...
| Año fiscal (@FiscalYear)               | Imprimir el ejercicio para la columna en forma numérica.                                                                                                                                                                                                                                            |
...
| Nombre de la unidad del árbol (@UnitName)         | Si se restringe una columna a una unidad concreta del organigrama, imprima el nombre de la unidad en el encabezado de columna.                                                                                                                                                                                     |
...
| Libreta codificada (@BookCode)                   | Imprima el código de libro que se especifica en la columna.                                                                                                                                                                                                                                             |
| Línea en blanco (@Blank)                     | Inserte una línea en blanco en el encabezado de columna.                                                                                                                                                                                                                                                       |
...
-   Si especifica los encabezados de columna que se superponen, se superponen cuando se imprimen en el informe. Se genera el informe, pero la siguiente advertencia aparece en ** estado de la cola de informes ** el campo: Los “encabezados de columna utilizando la base intersecan con otros encabezados de columna y pueden producir solapar el texto”. Por ejemplo, la definición de cabecera en B es la columna B a BASE+1, y la definición de cabecera en D de la columna es el BASE+1 F. en este caso, los encabezados se imprimen por arriba de uno a y son ilegibles. Siempre que BASE se use en una definición **Extensión desde/Extensión hasta**, asegúrese de ver el informe que se genera, para ver si los encabezados se superponen.
...
-   Para las columnas condicionales de impresión (** PB&lt;**, ** P=B&gt;**), títulos que abarcan condicionales se comportan como cualquier definición regular de encabezado de columna. Por ejemplo, si la condición es falsa, cualquier columna posterior coincidente para la condición de extensión inicia la impresión de cabecera.
...
| Control de impresión       |      | P=B&lt;       | PB&gt;        | P=B&lt;       | PB&gt;        | P=B&lt;       | PB&gt;        | P=B&lt;       | PB&gt;        | P=B&lt;       | PB&gt;        | P=B&lt;       | PB&gt;        |
...
| PB&lt;                         | Muestre una columna especificada solo si el período es menor que el período de la base.             |
| PB&gt;                         | Muestre una columna especificada solo si el período es mayor que el período de la base.             |
...
3.  Seleccione un tipo de columna en la lista. En la tabla siguiente se proporciona una descripción de los diferentes tipos de columnas.
    ...
    <tr class="odd">
    <td>ATTR</td>
    <td>Si su sistema contable admite atributos, se muestra una cuenta o un atributo de la transacción en la columna. Un atributo, que se debe aplicar a una única cuenta completa, extrae información de transacciones o de la cuenta subyacente de los datos financieros. los atributos de Cuenta- nivel muestran datos de la cuenta, y los atributos de temas nivel muestran los datos que se producen en el momento en que se registró la transacción. Si selecciona <strong>ATTR</strong> como el tipo de columna, especifique la categoría de atributo en <strong>Libreta codificada/categoría de atributo</strong> la fila de detalle de la definición de columna.</td>
    </tr>
    </tbody>
    </table>
...
| BASE+\# | Use el año \# que los años tras el cierre del base. Por ejemplo, para usar el tercer año después del año base, escriba **BASE+3**. |
| \#Dynamics | Use el año \# que los años del año antes base. Por ejemplo, para usar el último año, escriba **BASE-1**.                 |
...
| BASE+\#         | Use el período que \# los períodos tras el período base. Por ejemplo, para usar el tercer período después del período base, escriba **BASE+3**.                                                                                               |
| \#Dynamics         | Use el período que \# los períodos anteriores del período base. Por ejemplo, para usar el último período, escriba **BASE-1**.                                                                                                                 |
| Dynamics\#: BASE    | Use varios períodos, a partir de varios períodos anteriores al período de base hasta el período de la base. Por ejemplo, para usar los tres períodos anteriores y el período de base, escriba **BASE-3:BASE**.                                                |
| Dynamics: BASE+\#    | Use varios períodos, a partir del período de la base hasta varios períodos después del período de base. Por ejemplo, para usar el período de base y los siguientes dos períodos, escriba **BASE:BASE+2**.                                                  |
| \#debajo: BASE+\# | Use varios períodos, a partir de varios períodos anteriores al período de base hasta varios períodos después del período de la base. Por ejemplo, para usar los tres períodos previos, el período de la base y los siguientes dos períodos, escriba **BASE-3:BASE+2**. |
...
Puede ir más allá de los límites del ejercicio en cualquiera de las especificaciones de período, y puede mezclar años en un intervalo de períodos. Por ejemplo, puede especificar los períodos como ** BASE-5 ** (representar los últimos seis períodos) y ejecutar un informe que tenga un período base de 2. En este caso, el informe muestra datos para los primeros dos períodos de ejercicio especificado y los cuatro últimos períodos del ejercicio anterior.
...
3.  En la barra de fórmula encima del panel de navegación, o en la celda **Período**, complete la fórmula. Reemplazar cualquier signo de número (\#) con el valor adecuado.
...
| Ancho de columna                 | 30   |                      |
| ...                          |      |                      |
| Filtro de atributo             |      |  01:10 De Reference=\]\[ |

### <a name="dimension-filter-in-a-column-definition"></a>El filtro de la dimensión en una definición de la columna

Un filtro de la dimensión se usa para restringir la columna **FD** a los valores de dimensión específicos. El filtro puede incluir una dimensión única, un intervalo de dimensiones o a un grupo de dimensiones. El filtro también puede incluir sistemas del valor de dimensión. Dado que los valores de dimensión, pueden variar. A. \ las dimensiones financieras \ el sistema dimensión- basado no tiene que correspondan con una longitud precisa. El filtro se aplica, independientemente de si el informe incluye un organigrama. Puede usar un carácter comodín (\* o?) en cualquier puesto. Cuando se especifica varias cuentas, póngase una coma entre las cuentas, como en el ejemplo siguiente: ¿+Account=\[1200\],\[+Account= 1100\],\[Department= 01?\] Para recibir todos los departamentos para una cuenta concreta, puede excluir la dimensión de departamento del filtro de la dimensión. Por ejemplo, los dos filtros siguientes de la dimensión se gestionan de la misma manera:

-   +Account=\[1100\], departamento
-   +Account=\[\]1100

También puede usar cualquier combinación de caracteres alfanuméricos para una coincidencia exacta y puede definir dimensiones parciales. Por ejemplo, ** ubicación \[= 10\*\]** incluye todos los valores de dimensión de ubicación que comienzan por 10.
...
-   ** Visualización de la divisa ** permite especificar el tipo de divisa (natural, o funcional informe de errores) que las transacciones se muestran en. Esta función se hace referencia en ocasiones como traducción de la divisa. La traducción de la divisa es la capacidad de notificar importes de la contabilidad general en una divisa que puede no ser la divisa funcional de la empresa o la divisa en que se registró la transacción.
...
**Nota:** Las opciones que se describen aquí pueden variar, en función del sistema ERP. Para obtener más información, consulte [su documentación del sistema ERP de Microsoft (https://www.microsoft.com/en-us/download/details.aspx?id=5916)].
...
| Divisa funcional de la información de empresa | Vacía                | ** $2,310\*\*– ** el resultado muestra todos los datos en la divisa funcional que se especifica en la información de la empresa. **Nota:** este importe es la suma de todas las transacciones en divisa funcional. |
...
| \*       | Una C\*                | Multiplique el importe en la columna A por importe en la columna C.                                                                                                                                                                                     |
...
Puede crear una columna que muestre todos los valores en una columna especificada como un porcentaje de un número de la base. Por lo tanto, puede mostrar relaciones entre las filas, como un porcentaje de una fila de ventas o un porcentaje de una fila de los gastos totales. Para se multiplicará o dividir cada fila en una columna específica por una fila base, especifique la columna que se va a usar en el cálculo y, a continuación éntrela **\*BASEROW ** o ** ** /BASEROW. Por ejemplo, escriba ** C\*BASEROW ** o ** C/BASEROW **. ** Nota:** cuando usa un cálculo de la fila base en una definición de la columna, asegúrese de que cada definición de fila que se usa con esta definición de columna contiene el menos una fila de la base para los cálculos.
