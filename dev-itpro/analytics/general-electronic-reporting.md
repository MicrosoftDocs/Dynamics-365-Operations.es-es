...
-   Representa una sola herramienta común para el informe de errores electrónico en dominios diferentes, y reemplazo más de 20 distintos motores que causen una algún tipo de notificación electrónico para Microsoft Dynamics 365 para las operaciones.
-   Crea un formato de informe retirado de Dynamics 365 actual para la implementación de las operaciones. Es decir (el formato es aplicable para las diferentes versiones de las Dynamics 365 para las operaciones.)
...
-   Una asignación de modelo que vincula Dynamics seleccionado 365 para los orígenes de datos de las operaciones a los elementos individuales de un modelo de datos que especifique, en el tiempo de ejecución, el flujo de datos y las reglas de rellenado de los datos empresariales a un componente del modelo de datos.
...
-   Empresa -&gt; transacciones&gt; de pago del proveedor de dominio de proveedores
-   Cliente -&gt; empresa -&gt; Transacciones de pago de dominio de clientes
...
-   Puede utilizar Varios Dynamics 365 para los tipos de datos de operaciones como orígenes de datos para un modelo de datos. Por ejemplo, puede utilizar tablas, entidades de datos, métodos o enumeraciones.
...
-   Admite la transformación de Dynamics 365 para los datos de las operaciones en grupos necesarios, el filtro, la ordenación, y calcular datos, y también anexar con los campos calculados lógicos que están diseñados con Microsoft Excel- (como fórmulas para obtener más información, consulte [diseñador de fórmula en el informe de errores electrónico general-electronic-reporting-formula-designer.md] ()).

[Excel-![como editor de la fórmula (]. /media/pic-formula-1024x615.png])(. Están diseñados para cada dominio del negocio que se deba usar como origen de datos unificado para notificar el que aislantes que informan de implementación física de Dynamics 365 para los orígenes de datos de operaciones, y representa el componente del modelo de datos de /media/pic-formula.png) A conceptos y funcionalidades dominio- específicos de negocio en un formulario que cree el diseño inicial y el mantenimiento posterior de un formato de informes más de eficiencia.
...
-   ** COMPLETADO ** la versión se puede convertir a ** COMPARTIDA ** versión. Esta versión se publica en el CD y puede utilizarse en procesos globales de informes.
-   La versión **COMPARTIDO** puede convertirse en una versión **INTERRUMPIDA**. Esta versión se puede eliminar a continuación.

Las versiones que tienen ** COMPLETADO ** o ** COMPARTIDO ** estado están disponibles para otro intercambio de datos. En un componente que tiene estos estados se pueden realizar las siguientes acciones:

-   Se pueden serializadas en formato XML y exportar de Dynamics 365 para las operaciones como archivo en el formato XML.
-   Pueden reserialized de un archivo XML y se importen en Dynamics 365 para las operaciones como una nueva versión de un componente del ER.

#### <a name="component-date-effectivity"></a>Eficacia de la fecha del componente

Las versiones del componente de ER tienen fecha de vigencia. La fecha** Vigente desde **se puede definir para que un componente de ER especifique la fecha en la que el componente se hace efectivo para procesos de informes. La Dynamics 365 para la fecha de la sesión de las operaciones se usa para definir si un componente es válido para la ejecución. Si hay más de una versión válida para una fecha concreta, la última versión se usa para procesos de informes.

#### <a name="component-access"></a>Acceso del componente

El acceso a los componentes del formato de ER depende de la configuración del código de país o región ISO. Si el valor está en blanco para una versión seleccionada de una configuración del formato, un componente de formato puede tener acceso de cualquier Dynamics 365 para la empresa de operaciones en el tiempo de ejecución. Cuando este valor contiene los códigos de país o región ISO, un componente de formato está disponible únicamente de Dynamics 365 para las empresas de operaciones que tienen una dirección principal definida para uno de país o de los códigos de país ISO de un componente del formato. Diferentes versiones de un componente de formato de datos pueden tener distintos ajustes de los códigos de país o región ISO.

#### <a name="configuration"></a>Configuración

Una configuración de ER es la envoltura de un componente concreto de ER: **Modelo de datos **o **Formato**. Una configuración puede incluir diferentes versiones de un componente concreto de ER. Cada configuración se marca como perteneciente a un proveedor concreto de configuración. ** BORRADOR ** la versión de un componente de una configuración puede editarse si se ha seleccionado el propietario de la configuración como proveedor activas de los valores de ER de Dynamics 365 para las operaciones. Cada configuración de modelo contiene un componente de **Modelo de datos**. Una nueva configuración de formato se puede originar (derivar) de una configuración de modelo de datos específico. La configuración de formato que se crea se presentará en el árbol de configuración como un elemento secundario de la configuración del modelo de datos original. La configuración de formato que se crea contiene un componente **Formato **. El componente **Modelo de datos** de la configuración del modelo original se inserta automáticamente en el componente **Formato **de la configuración creada de formato secundario como origen de datos predeterminado. Una configuración de ER se comparte para Dynamics 365 para las empresas de operaciones.

#### <a name="provider"></a>Proveedor

El proveedor de ER es la identificación de parte que se usa para indicar el autor (propietario) de cada configuración de ER. ER le permite administrar la lista de proveedores de configuración. Aplicar formato a las configuraciones que se liberan para los documentos electrónicos como parte de Dynamics 365 para las operaciones que la solución se marca como propiedad de Microsoft ** ** el proveedor de la configuración.

#### <a name="repository"></a>Repositorio

Un repositorio de ER guarda las configuraciones de ER. Los siguientes tipos de repositorios de ER se admiten actualmente: ** Recursos de operaciones y ** ** proyecto de CD **. ** Recursos de operaciones ** un repositorio proporciona acceso a la lista de configuraciones que se liberar como parte de Dynamics 365 para la solución de las operaciones por Microsoft como proveedor de la configuración del ER. Estas configuraciones se pueden importar en Dynamics 365 actual para las operaciones citan como ejemplo y utilizadas para el informe de errores electrónico. También se pueden utilizar para localizaciones/personalizaciones adicionales. Un repositorio del **proyecto de LCS **proporciona acceso a la lista de configuraciones de un proyecto de LCS específico (biblioteca de los activos del proyecto de LCS) que se seleccionó en la etapa de registro del repositorio. El ER permite cargar las configuraciones compartidas de Dynamics 365 actual para la instancia un detalle ** proyecto de las operaciones de CD ** repositorio. También puede importar las configuraciones de detalle ** un proyecto de CD ** repositorio en Dynamics 365 actual para la instancia de las operaciones. ** Proyecto de CD ** los repositorios necesarios se pueden registrar de forma individual para cada proveedor de la configuración de Dynamics 365 actual para la instancia de las operaciones. Cada repositorio se puede dedicar a un proveedor de configuración específico.

## <a name="supported-scenarios"></a>Escenarios admitidos
### <a name="building-a-data-model"></a>Creación de un modelo de datos

ER proporciona un diseñador de modelo que puede usar para crear un modelo de datos para un dominio concreto de negocio. Todas las entidades específicas de empresa de dominio y las relaciones entre ellas se pueden presentar en un modelo de datos como una estructura jerárquica. En la siguiente ilustración se muestra un ejemplo de este tipo de modelo de datos (el modelo de datos de dominio de pago). [ejemplo de![del modelo de datos (]. /media/pic-data-model-1024x550.png])(. /media/pic-data-model.png) A quedar familiares con los detalles de este escenario, juegan ** modelo de datos específica de dominio de diseño de ER ** la guía de la tarea (parte ** 7.5.4.3 adquiere/desarrolla los componentes de servicio o la solución de IT (10677) ** del proceso empresarial).

### <a name="translating-data-model-content"></a>Traducción del contenido del modelo de datos

El contenido del modelo de datos (las etiquetas y las descripciones) se pueden traducir a otros idiomas que Dynamics 365 para las operaciones admita. Desea traducir el contenido del modelo de datos por las siguientes razones:
...
-   En el tiempo de ejecución, para hacer que el contenido más sencillos mostrando los indicadores y la ayuda de los parámetros de tiempo de ejecución, y también configurados mensajes de validación (errores y advertencias), en el idioma que Dynamics actualmente abierta sesión 365 para el usuario de las operaciones prefiere

La ilustración siguiente muestra un ejemplo de cómo se puede traducir el contenido del modelo de datos del inglés al japonés. [contenido del modelo de datos![en inglés (]. /media/pic-translate-en-1024x495.png])(. /media/pic-translate-en.png) [contenido del modelo de datos![a traducido japonés] (. /media/pic-translate-ja-1024x495.png])(. /media/pic-translate-ja.png)

### <a name="configuring-data-model-mappings"></a>Configuración de asignación de modelo de datos

El ER proporciona un diseñador modelo de asignación que deje los modelos de datos de mapa de los usuarios que han diseñado a Dynamics 365 específica para los orígenes de datos de las operaciones. La ilustración siguiente muestra un ejemplo de este tipo de asignación de modelo de datos (**Transferencia de crédito SEPA** asignación del modelo de datos de dominio de pago). [ejemplo![de una asignación del modelo de datos (]. /media/pic-model-mapping-1024x551.png])(. /media/pic-model-mapping.png) Para pasar a estar familiarizado con los detalles de este escenario, jugar ** el ER define la asignación de modelo y seleccione orígenes de datos y ** ** modelo de datos de asignación de ER orígenes de datos seleccionados ** guías de la tarea (parte ** 7.5.4.3 adquiere/desarrolla los componentes de servicio o la solución de IT (10677) ** del proceso empresarial).
...
El ER puede almacenar un modelo de datos diseñado junto con asignaciones asociadas de datos como configuración del modelo de Dynamics 365 actual para la instancia de las operaciones. En la siguiente ilustración se muestra un ejemplo de este tipo de configuración de modelo de datos (la configuración del modelo de pago). [ejemplo![desde una configuración del modelo de datos (]. /media/pic-model-configuration-1024x585.png])(. /media/pic-model-configuration.png) A quedar familiares con los detalles de este escenario, juegan ** modelo de datos de asignación de ER orígenes de datos seleccionados ** la guía de la tarea (parte ** 7.5.4.3 adquiere/desarrolla los componentes de servicio o la solución de IT (10677) ** del proceso empresarial).

### <a name="building-a-format-that-uses-a-data-model-as-a-base"></a>Creación de un formato que usa un modelo de datos como base

ER admite un diseñador de formato que puede usar para crear el formato de un documento electrónico determinado para un dominio empresarial seleccionando el componente modelo como base. El mismo diseñador del formato de ER le permite asignar un formato que crea a la asignación del modelo de datos de un dominio seleccionado como origen de datos. En la siguiente ilustración se muestra un ejemplo de este tipo de formato (la configuración de formato que admite el formato de pago **BACS** para el Reino Unido). [ejemplo de![del formato que tiene un modelo de datos como base (]. /media/pic-format-1024x690.png])(. /media/pic-format.png) A quedar familiares con los detalles de este escenario, juegan ** formato específico del dominio de diseño de ER ** la guía de la tarea (parte ** 7.5.4.3 adquiere/desarrolla los componentes de servicio o la solución de IT (10677) ** del proceso empresarial).
...
El diseñador de formato de ER se puede utilizar para generar un documento electrónico concreto en formato de hoja de cálculo OPENXML. La ilustración siguiente se muestra un ejemplo de este tipo de formato (una configuración del formato para generar la hoja de cálculo de OPENXML con los detalles de un diario de pago seleccionado): [Imagen-ER-FORMATO- Excel![] (. /media/pic-er-format-excel.jpg])(. /media/pic-er-format-excel.jpg) A quedar familiares con los detalles de este escenario, juegan ** el ER crear una configuración para informes en formato de OPENXML ** la guía de la tarea (parte ** 7.5.4.3 adquiere/desarrolla los componentes de servicio o la solución de IT (10677) ** del proceso empresarial). Use consultado por debajo del archivo de Excel como una plantilla del formato de diseño de ER para completar el paso de la importación de una plantilla de formato de este manual de la tarea: [Plantilla del informe de pago (SampleVendPaymWsReport.xlsx)](https://go.microsoft.com/fwlink/?linkid=845202)
...
El ER puede almacenar un formato diseñado junto con las asignaciones configuradas de datos como configuración del formato de Dynamics 365 actual para la instancia de las operaciones. La ilustración anterior muestra un ejemplo de este tipo de configuración de formato (**BACS (Reino Unido)**, que es un elemento secundario de la configuración de **Modelo de pago**). Para familiarizarse con los detalles de este escenario, reproduzca la guía de tareas **CE Diseñar el formato específico de dominio** (parte del proceso empresarial **7.5.4.3 Adquirir/Desarrollar componentes de solución/servicios de la TI (10677)**).

### <a name="configuring-dynamics-365-for-operations-to-start-to-use-a-created-format-internally"></a>Dinámica de configuración 365 para operaciones que puedan utilizar un formato creado internamente

La Dynamics 365 para las operaciones se puede configurar empezar a usar un formato creado para generar informes electrónicos. La referencia a la configuración creada de formato se debe definir en la configuración de un determinado dominio. Por ejemplo, para empezar a usar una configuración del formato de ER para pagos electrónicos a proveedores en formato de BACS, la configuración del formato debe hacer referencia en formas de pago específicas, tal como se muestra en las siguientes ilustraciones: 

[![Configuración del formato de Reino Unido (BACS)](media/ger-bacs-uk-format-configuration.png) 

[![Referencia al formato de Reino Unido (BACS) en un método de pago](media/ger-bacs-uk-format-method.png) 
...
El ER permite importar los componentes de ER de CD a Dynamics 365 actual para la instancia de las operaciones. Se requiere un repositorio del tipo **Proyecto de LCS**. Cuando han importado un componente de ER de CD a Dynamics 365 actual para las operaciones cite como ejemplo, el propietario de la instancia hace un consumidor de servicio proporcionado por el propietario) (autor del componente importado. Por ejemplo, si el estilo un componente de formato para generar un documento electrónico específico de Dynamics 365 para las operaciones en un formato específicas del país/región (escenario de localización), ha asumido que el consumidor de servicio podrá recopilar las actualizaciones que se realicen a ese formato, para mantener compatible con requisitos legislativos. Para familiarizarse con los detalles de este escenario, reproduzca la guía de tareas **ER Importar una configuración de Lifecycle Services** (parte del proceso empresarial **7.5.4.3 Adquirir/Desarrollar componentes de solución/servicios de la TI (10677)**).
...
## <a name="list-of-er-configurations-that-are-delivered-in-the-dynamics-365-for-operations-solution"></a>Lista de configuraciones de ER que se suministran en Dynamics 365 para la solución de operaciones
