...
description: Este tema proporciona una visión general de las nuevas capacidades de clienteling disponibles en la aplicación de tienda.
author: bebeale
manager: AnnBe
ms.date: 11/01/19
ms.topic: article
...
ms.locfileid: "2774049"
---
# <a name="clienteling-overview"></a>Visión general de clienteling
...
1. En el espacio de trabajo **Administración de características**, filtre las características por el módulo **Retail y Commerce**.
    ...
    ![Clienteling en la lista de características del módulo Retail y Commerce](./media/Enable_clienteling.png "Clienteling en la lista de características del módulo Retail y Commerce")

2. Activa la función **Clienteling**; para ello seleccione **Habilitar ahora**.
3. En la página **Parámetros comerciales**, en la pestaña **Secuencia numérica**, seleccione la fila **Identificador de libro de clientes**. Luego, en el campo **Código de secuencia numérica**, seleccione una secuencia numérica. El sistema utilizará esta secuencia numérica para asignar un Id a los libros de clientes.
...
7. Para capturar las actividades que se producen en el PDV, defina los tipos de actividad en la página **Tipos de actividad** (**Ventas al por menor \> Clientes \> Tipos de actividad**).

    > [!NOTE]
    > Los tipos de actividad son extraídos por Retail Server cuando realiza una llamada en tiempo real por primera vez. Después de que las actividades se extraigan, se almacenan en la memoria caché durante algunas horas. Si cambia los tipos de actividad, espere hasta que la caché ya no sea válida. Como alternativa, para los entornos de no producción, reinicie el servicio de Retail Server.
...
## <a name="turn-on-the-integration-of-customer-insights-with-retail"></a>Activar la integración de Customer Insights con Retail

Para activar la integración de Customer Insights con Retail, debe asegurarse de que tenga una instancia activa de Customer Insights en el inquilino donde Retail está aprovisionado. También debe tener una cuenta de usuario de Azure Active Directory (Azure AD) que tenga una suscripción de Azure.
...
4. Conecte el acceso a Azure Key Vault desde Retail. Para completar este paso, debe tener un identificador y un secreto de la aplicación. La aplicación puede ser la misma solicitud que ha creado en el paso 1, o puede ser una nueva aplicación. (Es decir, puede usar la aplicación que creó en el paso 1 para el acceso a Key Vault y el acceso al servicio de Customer Insights, o puede crear una aplicación única para cada tipo de acceso.) Para obtener instrucciones, consulte [Credenciales principales del servicio de tienda de Azure Stack Key Vault](https://docs.microsoft.com/azure-stack/user/azure-stack-key-vault-store-credentials?view=azs-1908#create-a-service-principal).
5. En Retail Headquarters, vaya al **Administración del sistema \> Configuración \> Parámetros de Key Vault** e introduzca la información requerida para el Key Vault. A continuación, en el campo **Cliente de Key Vault**, especifique el identificador de la aplicación que usó en el paso 4, de modo que Retail pueda acceder a los secretos del almacén de claves.
...
7. En Retail, en la página **Parámetros de Retail**, en la pestaña **Clienteling**, en la ficha desplegable **Dynamics 365 Customer Insights**, siga estos pasos:
    ...
    5. En el campo **Id. de cliente alternativo**, especifique la propiedad de Customer Insights que esté asignada al número de cuenta del cliente. (En Retail, el número de cuenta del cliente es el ID de cliente.)
