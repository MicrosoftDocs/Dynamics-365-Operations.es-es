...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="a1e12-103">Preguntas más frecuentes sobre los arranques con datos de empresa</span><span class="sxs-lookup"><span data-stu-id="a1e12-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]


## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="a1e12-104">¿Por qué necesito el arranque?</span><span class="sxs-lookup"><span data-stu-id="a1e12-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="a1e12-105">Es posible que tenga una instancia de Common Data Service ya existente u otra aplicación de Dynamics 365 con datos empresariales, y desee habilitar la conexión de escritura doble con ella.</span><span class="sxs-lookup"><span data-stu-id="a1e12-105">You might have an existing Common Data Service or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="a1e12-106">En este caso, tiene que arrancar Common Data Service u otros datos de aplicaciones de Dynamics 365 con información de la empresa antes de habilitar la conexión de escritura doble.</span><span class="sxs-lookup"><span data-stu-id="a1e12-106">In this case, you need to bootstrap Common Data Service or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="a1e12-107">¿Cuándo debo utilizar el arranque?</span><span class="sxs-lookup"><span data-stu-id="a1e12-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="a1e12-108">Debe usar el arranque antes de habilitar las asignaciones de entidad de escritura doble (durante el paso 5).</span><span class="sxs-lookup"><span data-stu-id="a1e12-108">You should use bootstrapping before enabling dual-write entity maps (during step #5).</span></span>  
1. <span data-ttu-id="a1e12-109">Para configurar la conexión de escritura doble entre instancias de la aplicación de Finance and Operations y Common Data Service u otra aplicación de Dynamics 365, inicie sesión en la aplicación de Finance and Operations como administrador.</span><span class="sxs-lookup"><span data-stu-id="a1e12-109">To setup the dual-write connection between instances of your Finance and Operations app and the Common Data Service or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="a1e12-110">Vaya al módulo **Administración de datos** y haga clic en el botón **Escritura doble**.</span><span class="sxs-lookup"><span data-stu-id="a1e12-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="a1e12-111">Esto inicia el **Integrador de datos**.</span><span class="sxs-lookup"><span data-stu-id="a1e12-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="a1e12-112">Cree la conexión de escritura doble para una o varias empresas.</span><span class="sxs-lookup"><span data-stu-id="a1e12-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="a1e12-113">![Crear conexión de escritura doble](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="a1e12-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="a1e12-114">Habilite la asignación de entidad **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="a1e12-114">Enable the **Cdm_companies** entity map.</span></span> <span data-ttu-id="a1e12-115">Esto sincroniza empresas de la aplicación de Finance and Operations con Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="a1e12-115">This synchronizes companies from the Finance and Operations app to Common Data Service.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="a1e12-116">![Habilitar la asignación de entidad](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="a1e12-116">![Enable the entity map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="a1e12-117">Ejecute el código de arranque de muestra en la instancia de Common Data Service u otra instancia de la aplicación de Dynamics 365.</span><span class="sxs-lookup"><span data-stu-id="a1e12-117">Run the sample bootstrapping code on the Common Data Service or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="a1e12-118">Cuando se haya efectuado el arranque y el sistema está listo para la sincronización en directo, habilite las asignaciones de entidades.</span><span class="sxs-lookup"><span data-stu-id="a1e12-118">When the bootstrapping is done and the system is ready for the live sync, enable the entity maps.</span></span>  

    <span data-ttu-id="a1e12-119">Habilitar las asignaciones de entidades desencadena la sincronización de datos inicial para las asignaciones de entidades habilitadas.</span><span class="sxs-lookup"><span data-stu-id="a1e12-119">Enabling the entity maps triggers the initial data sync for the enabled entity maps.</span></span> <span data-ttu-id="a1e12-120">Los datos correspondientes a las empresas elegidas en la conexión de escritura doble se sincronizan entre la aplicaciones de Finance and Operations y Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="a1e12-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Common Data Service.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="a1e12-121">¿Cómo utilizo el ejemplo de código?</span><span class="sxs-lookup"><span data-stu-id="a1e12-121">How to I use the code sample?</span></span>
<span data-ttu-id="a1e12-122">El código de muestra es una aplicación C# que puede cargar en Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="a1e12-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="a1e12-123">Toma dependencias de paquete NuGet en el SDK de Common Data Service que puede actualizar mediante las herramientas de Visual Studio estándar.</span><span class="sxs-lookup"><span data-stu-id="a1e12-123">It takes NuGet package dependencies on the Common Data Service SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="a1e12-124">Después de descomprimir y abrir la solución en Visual Studio y de restablecer los paquetes NuGet, busque **TODO** en el código.</span><span class="sxs-lookup"><span data-stu-id="a1e12-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="a1e12-125">Cada decisión que necesita tomar sobre cómo desea arrancar información de la empresa es anotada por un **TODO**, con código de muestra para una implementación canónica.</span><span class="sxs-lookup"><span data-stu-id="a1e12-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="a1e12-126">El código de muestra muestra sólo una de varias maneras en que es posible clasificar en categorías los registros de entidad por empresa.</span><span class="sxs-lookup"><span data-stu-id="a1e12-126">The sample code shows only one of many ways you might categorize entity records by company.</span></span> <span data-ttu-id="a1e12-127">Al cambiar la lógica de las secciones **TODO**, puede crear su clasificación personalizada.</span><span class="sxs-lookup"><span data-stu-id="a1e12-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="a1e12-128">¿Qué debo esperar?</span><span class="sxs-lookup"><span data-stu-id="a1e12-128">What should I expect?</span></span>
<span data-ttu-id="a1e12-129">De forma predeterminada, la aplicación de ejemplo permite proporcionar un diccionario de asignaciones de código de unidad de empresa a empresa.</span><span class="sxs-lookup"><span data-stu-id="a1e12-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="a1e12-130">Toda entidad que arranque con un campo **OwningBusinessUnit** automáticamente se establece para utilizar la empresa especificada.</span><span class="sxs-lookup"><span data-stu-id="a1e12-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="a1e12-131">Cualquier entidad sin un campo **OwningBusinessUnit**, como producto, establecerá la empresa basándose en la asignación con un valor de unidad de negocio vacío.</span><span class="sxs-lookup"><span data-stu-id="a1e12-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="a1e12-132">La aplicación de la consola espera un parámetro, **–simulate** o **–apply**.</span><span class="sxs-lookup"><span data-stu-id="a1e12-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="a1e12-133">Si usa el parámetro de la línea de comando **–simulate**, no se actualiza ningún dato.</span><span class="sxs-lookup"><span data-stu-id="a1e12-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="a1e12-134">Solo se generan los archivos **simulation_<entityname>.csv** en el mismo directorio que la herramienta, uno para cada entidad que habría sido actualizada.</span><span class="sxs-lookup"><span data-stu-id="a1e12-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="a1e12-135">Puede revisar estos archivos iterativamente mientras trabaja para asegurarse de que el código actualiza los valores de la empresa como esperado.</span><span class="sxs-lookup"><span data-stu-id="a1e12-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="a1e12-136">Cuando termine con las actualizaciones simuladas, use el parámetro **–apply**.</span><span class="sxs-lookup"><span data-stu-id="a1e12-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="a1e12-137">Esto actualiza todos los registros que tienen actualmente un valor incorrecto de empresa, en lotes de 1000 registros al mismo tiempo (de forma predeterminada).</span><span class="sxs-lookup"><span data-stu-id="a1e12-137">This updates all records that currently have an incorrect company value, in batches of 1000 records at a time (by default).</span></span> <span data-ttu-id="a1e12-138">El código, tal y como se proporciona, es idempotente, es decir, puede volverlo a ejecutar y sólo actualizará las empresas incorrectamente asignadas.</span><span class="sxs-lookup"><span data-stu-id="a1e12-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="a1e12-139">Al ejecutarse con **–apply**, el código genera archivos CSV de los cambios realizados, que se denominan **applied_<entityname>.csv**.</span><span class="sxs-lookup"><span data-stu-id="a1e12-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
