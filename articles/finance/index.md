---
title: Página principal de Finance
description: En este artículo se proporciona una lista de los temas de ayuda y de otros recursos para las características de gestión financiera en Microsoft Dynamics 365 Finance.
author: kfend
ms.date: 11/04/2021
ms.topic: index-page
ms.prod: ''
ms.technology: ''
audience: Application User
ms.reviewer: kfend
ms.custom:
- "206434"
- intro-internal
ms.assetid: 8a6238bd-9d6f-48d9-8752-3966836364a4
ms.search.region: Global
ms.author: kfend
ms.search.validFrom: 2016-02-28
ms.dyn365.ops.version: AX 7.0.0
ms.openlocfilehash: 49d36dc5818bdedfc764512e2f06c597c31e5b2a
ms.sourcegitcommit: d3f7a56eaf788d223ece4cedac4a319eaf5f6112
ms.translationtype: HT
ms.contentlocale: es-ES
ms.lasthandoff: 09/19/2022
ms.locfileid: "9539245"
---
# <a name="finance-home-page"></a>Página principal de Finance

[!include [banner](includes/banner.md)]

En este artículo se proporciona una lista de los temas de ayuda y de otros recursos para las características de gestión financiera en Microsoft Dynamics 365 Finance. 

Seleccione un área de características para obtener más información al respecto.

:::row:::
    :::column:::
        - [Proveedores](accounts-payable/accounts-payable.md) 
        - [Clientes](accounts-receivable/accounts-receivable.md)
        - [Arrendamiento de activos](asset-leasing/asset-leasing-homepage.md)
        - [Gestión presupuestaria](budgeting/budgeting-overview.md) 
        - [Gestión de efectivo y bancos](cash-bank-management/cash-bank-management.md)
        - [Contabilidad de costes](cost-accounting/cost-accounting-home-page.md)
    :::column-end:::
    :::column:::
        - [Informes financieros](../fin-ops-core/dev-itpro/analytics/financial-reporting-intro.md?toc=/fin-and-ops/toc.json)
        - [Activos fijos](fixed-assets/fixed-assets.md)
        - [Finance Insights](finance-insights/finance-insights-home-page.md)
        - [Contabilidad general e informes financieros](general-ledger/general-ledger.md) 
        - [Sector público](public-sector/public-sector-functionality.md) 
    :::column-end:::
:::row-end:::


## <a name="additional-resources"></a>Recursos adicionales

### <a name="blogs"></a>Blogs

- [Blog de Microsoft Dynamics 365](https://community.dynamics.com/b/msftdynamicsblog?c=Enterprise)
- [Blog de Financials](https://community.dynamics.com/365/financeandoperations/b/financials) 
- [Blog de la comunidad de Microsoft Dynamics Operations Partner](https://community.dynamics.com/partner/b/operationspartnercommunityblog)

### <a name="videos"></a>Vídeos

Consulte los vídeos de procedimientos que se encuentran ahora disponibles en el [canal de YouTube de Microsoft Dynamics 365](https://www.youtube.com/channel/UCJGCg4rB3QSs8y_1FquelBQ).

### <a name="countryregion-functionality"></a>Funcionalidad por país/región

Las normativas por país/región afectan a la configuración de impuestos y a otras áreas de la administración financiera. Para obtener más información sobre la funcionalidad específica de región o país, consulte [Características normativas y de localización](../fin-ops-core/dev-itpro/lcs-solutions/country-region.md?toc=/fin-and-ops/toc.json).

### <a name="additional-content"></a>Contenido adicional

La funcionalidad de Supply Chain Management cubre partes del proceso de la compra al pago que incluyen:

   - Solicitud
   - Pedido 
   - Recepción
   - Facturación
   - Pago de los bienes y servicios que su organización compra 
    
Para obtener más información acerca de las funcionalidades para gestionar compras, inventario y fabricación, consulte la [página principal de Supply Chain Management](../supply-chain/index.md).

## <a name="elearning-courses"></a>Cursos de eLearning

Para cursos y formación en línea, consulte [Rutas de aprendizaje de Dynamics 365 Finance](/training/browse/?products=dynamics-finance&resource_type=learning+path).

[!INCLUDE[footer-include](../includes/footer-banner.md)]
