...
Las dimensiones financieras no están actualmente en CDS para aplicaciones y como consecuencia no forman parte de la plantilla predeterminada. Esta entidad se planifica, pero no hay una escala de tiempo de liberación disponible.
...
Los campos se pueden agregar o quitar de la tarea de integración. No todos los campos de datos que existen en la entidad CDS para aplicaciones (CDS 2.0) se rellenarán desde Core HR.
...
La primera ejecución del integrador de datos es una siempre una ejecución completa. Las ejecuciones posteriores se basan en seguimiento de cambios. Cuando se ejecuta una ejecución de errores, se extraen los registros en el ámbito de la ejecución y se envían los cambios más recientes de CDS.
...
Sí, los campos se pueden agregar o quitar de la plantilla existente. La plantilla se puede modificar para incluir datos adicionales de otras entidades de CDS para aplicaciones. La entidad debe estar en CDS para aplicaciones para que se incluya en la plantilla. 
...
- La transferencia de datos produjo una extracción de registros duplicada en el origen (CDS).

- La transferencia de datos tiene valores nulos para campos que son necesarios en Finance and Operations. Compruebe que los datos que están en CDS y cumplen con los requisitos de Finance and Operations.
...
La integración en Finance and Operations es un proceso de dos pasos. En primer lugar, compruebe que los datos de Talent estén actualizados y disponibles en CDS. Esto es una sincronización prácticamente en tiempo real y se puede comprobar en PowerApps buscando los datos en las entidades de datos.

![Datos en CDS](media/DataInCDS.png)

Si los datos no aparecen como se esperaba en CDS, compruebe que la entidad se admita en la integración. Para incluir datos adicionales en CDS, se necesitará un cambio en el lado de Microsoft.
...
La secuencia numérica para **Identificador de la ubicación** usa el mismo patrón en Talent y en Finance and Operations. La secuencia numérica tiene que ser única en ambos lados para que no haya colisiones de dirección al integrar datos de CDS en Finance and Operations.
...
Asegúrese de que está realizando asignaciones a las entidades jurídicas correctas. La sincronización de entidades jurídicas no es parte de la plantilla predeterminada, por lo que se espera que cada entidad jurídica que está presente en Talent y CDS también esté en Finance and Operations.
...
- Integrador de datos (DI): 
  ...
  - [Integre datos en Common Data Service para aplicaciones](https://docs.microsoft.com/en-us/powerapps/administrator/data-integrator)
  ...
  - [Respuesta a solicitudes de DSR para registros generados por el sistema en PowerApps, Microsoft Flow y Common Data Service para aplicaciones](https://docs.microsoft.com/en-us/powerapps/administrator/powerapps-gdpr-dsr-guide-systemlogs)
